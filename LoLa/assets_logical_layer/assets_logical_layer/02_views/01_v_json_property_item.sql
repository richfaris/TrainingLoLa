/* v_json_property_item [v_property_items, v_policy_type_items, v_policy_type_items_forms] */
create or replace algorithm = merge view v_json_property_item as
select
  i.property_item_id,
  i.property_id,
  y.item_order,
          ...
          and if(f.mortgagee_types_condition is null, 1,
                 (select json_overlaps(
                    ...
                    (select json_arrayagg(m.mortgage_type)
                     ...
                     where m.relationship = 'mortgagee' and m.property_id = i.property_id)
                  )
                 )
          )
        ) f
